 ...
 * 
 * @author mxl
 * @version $ OverrideDirective.java v1.0, 2017年4月25日 下午7:03:06 mxl Exp $
 */
public class OverrideDirective extends Directive {
	...
	public boolean render(InternalContextAdapter context, Writer writer,
		...
		OverrideNodeWrapper override = (OverrideNodeWrapper) context
				.get(DirectiveUtil.getOverrideVariableName(name));
		if (override == null) {
			Node body = node.jjtGetChild(1);
			context.put(DirectiveUtil.getOverrideVariableName(name) ,
					new OverrideNodeWrapper(body));
		} else {
			OverrideNodeWrapper current = new OverrideNodeWrapper(
					node.jjtGetChild(1));
	...
	static class OverrideNodeWrapper extends SimpleNode {
		...
		public boolean render(InternalContextAdapter context, Writer writer)
			...
			OverrideNodeWrapper preNode = (OverrideNodeWrapper) context
					.get(DirectiveUtil.OVERRIDE_CURRENT_NODE);
			try {
